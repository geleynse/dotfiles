...
Required mods:
  ...
  - LTN Combinator Modernized (2.0-compatible; tested against 2.2.5)
  - Bob's Inserters (for bob-red-inserter on the train-adjacent row)

Optional: LTN Manager (tested against 0.5.1) for in-game station visibility.
...
#
# The ltn-combinator's slot 1 is seeded with ltn-network-id = -1 (LTN's default
# "all networks" value) so the combinator always emits at least one signal.
# Rationale: LTN Manager 0.5.1 ipairs() the lamp's get_signals() result without
# a nil-guard, and Factorio 2.0's get_signals() returns nil (not an empty table)
# when the queried wires carry no signals.
#
...
#   y=-1.5: combinators + output inserters. Provider has just decider #2 (+4).
#           Receiver/dual-north has extra pole (-1.5) + request inverter (+2) +
#           cap decider A (+4) + cap decider B (+6). Cap deciders emit on OUT_RED
#           to lamp RED — the lamp's red wire is a separate network from GREEN,
#           so decider output does not feed back into the request inverter on
#           GREEN even though LTN sums red+green at read-time.
#           Output inserters span x=+7.5..+24.5 (18 per station).
#   y=-0.5: chest row + LTN head (ltn-comb/lamp) + CC/arith/decider #1 inline
...
# North combinator positions. Chest row (y=-0.5) holds the CC, stop-RED isolator
# arith and decider #1 on every station. Output-inserter row (y=-1.5) holds either
# decider #2 (provider) or the request-cap circuit (receiver/dual-north, below).
X_CC_N = 1.5
Y_CC_N = -0.5
X_ARITH_N = 3
Y_ARITH_N = -0.5
...
# Request inverter (×-1 arith) — inverts the negative request signal from the
# ltn-combinator into a positive "still wanted" quantity for the cap deciders.
X_REQ_INV_N = 2
Y_REQ_INV_N = -1.5

# Request-cap deciders on the output-inserter row. decider #3 passes the current
# request when chest is overloaded; decider #2 passes the chest count when below.
X_DECIDER3_N = 6
Y_DECIDER3_N = -1.5

# Extra power pole for the receiver/dual-north cap circuit — the request inverter
# at x=+2 falls outside the supply radius of the chest-row pole at +6.5.
X_EXTRA_POLE_N = -1.5
Y_EXTRA_POLE_N = -1.5

# Output inserter drop_position — offsets (relative to inserter) that land items
# on a two-lane belt immediately north of the inserter row. Copied from the
# user's working in-game station (Bob's Inserters mod, adjustable drop slider).
# Alternates by inserter index within each wagon: even i → +0.203125, odd i →
# -0.19921875. Y offset is always -0.80078125 (one tile north of inserter).
OUTPUT_INSERTER_DROP_Y = -0.80078125
OUTPUT_INSERTER_DROP_X_EVEN = 0.203125
OUTPUT_INSERTER_DROP_X_ODD = -0.19921875
...
def build_passthrough(eid: int, position: dict, direction: int = EAST) -> Entity:
    """Arithmetic combinator: Each * 1.

    Used as a network isolator when LTN entities must not share a logic network
    with each other, but their signals still need to be combined downstream.
    """
    return Entity(
        entity_number=eid,
        name="arithmetic-combinator",
...
def build_each_compare_decider(eid: int, position: dict, comparator: str, *,
                               direction: int = EAST,
                               copy_from_red: bool = True) -> Entity:
    """Decider: (red Each <cmp> green Each) AND (red Each > 0), output Each copied
    from selected input.

    The second clause is critical. Without it, the decider fires on every signal
    present on green — including the ltn-combinator's control signals (network-id,
    thresholds, priorities) routed through the request inverter. Control signals
    have red=0 (the chest chain doesn't carry them), so an inverter-negated control
    signal like ltn-network-id=-1 would satisfy `red(0) > green(-1)` and be echoed
    onto lamp RED, where it then cancels the real ltn-combinator signal on lamp
    GREEN. LTN's get_signals() drops zero-sum signals, leaving the lamp appearing
    empty and UpdateStop bailing before setLamp — lamp stays at initial white.

    Requiring red > 0 restricts the decider to items actually present in the chest
    chain, which is exactly what the request cap should do.
    """
    return Entity(
        entity_number=eid,
        name="decider-combinator",
        ...
        control_behavior={
            "decider_conditions": {
                "conditions": [
                    {
                        ...
                        "first_signal_networks": {"red": True, "green": False},
                        "second_signal": virtual_signal("signal-each"),
                ...
                "outputs": [{
                    "signal": virtual_signal("signal-each"),
                    "copy_count_from_input": True,
                    "networks": {"red": copy_from_red, "green": not copy_from_red},
                }],
            },
        },
    )
...
def build_ltn_combinator(ids, ltn_tags):
    ...
    Slot 1 is pre-filled with `ltn-network-id = -1` (LTN's default "all networks"
    value, and the slot reserved for this signal by LTN Combinator Modernized).
    This guarantees the combinator always emits at least one signal, which flows
    through to the input lamp. LTN Manager 0.5.1 calls `ipairs()` on the lamp's
    `get_signals()` output unguarded; Factorio 2.0 returns nil when the queried
    wires carry no signals, and nil crashes ipairs. Keeping one always-on signal
    on the lamp's input sidesteps the crash.
    """
    eid = ids.next()
    ent = Entity(
        entity_number=eid,
...
def _build_ltn_head(ids, station_name, ltn_tags, entities, wires):
    """Build stop + input lamp + output_CC + ltn-combinator. Wires ltn GREEN to
    lamp GREEN directly so every LTN control signal (network-id, thresholds,
    priorities) reaches the lamp without crossing a combinator.

    Request-cap stations (receiver, dual-north) rely on the fact that the lamp's
    red wire is a SEPARATE network from its green wire. The cap deciders write to
    lamp RED, and the request inverter reads ltn GREEN — so decider output does
    not feed back into the inverter and no oscillation forms.
...
def _build_north_side(
    ...
    *,
    output_cc_id, stop_id, input_lamp_id, ltn_id,
    allowlist,
    ...
    Layout (stop-rel):
      y=-1.5: extra pole + request inverter + 2 inventory-cap deciders + output inserters
    ...
    Poles:
      ...
      combinator row: x=-1.5 (powers the request inverter)

    Wiring (receiver / unload):
      stop RED → north arith(×1) RED → decider #1 RED
      CC GREEN → decider #1 GREEN
      ltn-combinator GREEN → lamp GREEN (direct; LTN sees all control signals here)
      ltn-combinator GREEN → request inverter(×-1) → positive request GREEN
      chest chain RED + positive request GREEN → min(chest, request) → lamp RED (NOT green)
      decider #1 OUT_RED → first chest-row pole RED → bob-red chain (cross-row to y=+0.5)
      chest chain RED only (no green chain)

    The cap deciders deliberately output on lamp RED — not lamp GREEN — because
    the lamp's red and green wires are separate circuit networks at the lamp.
    LTN sums red+green when reading the lamp's signals, so routing the cap
    inventory through red still reaches LTN with the right net value; but since
    the request inverter reads ltn GREEN, putting the decider output on RED
    keeps it off the inverter's input network and prevents a feedback oscillation.
    ...
    # Isolate stop cargo from the LTN output network. LTN flags a short circuit if
    # stop/output/input share any logic network, even indirectly through a combinator input.
    arith_id = ids.next()
    entities.append(build_passthrough(arith_id, {"x": X_ARITH_N, "y": Y_ARITH_N}, direction=EAST))

    decider1_id = ids.next()
    entities.append(build_provider_decider(decider1_id, {"x": X_DECIDER1_N, "y": Y_DECIDER1_N}, direction=EAST))

    # North requester inventory cap on output row: request inverter + two deciders that
    # together output min(chest_count, current_request) to the lamp.
    req_inv_id = ids.next()
    entities.append(build_inverter(req_inv_id, {"x": X_REQ_INV_N, "y": Y_REQ_INV_N}, direction=EAST))
    decider2_id = ids.next()
    entities.append(build_each_compare_decider(decider2_id, {"x": X_DECIDER2_N, "y": Y_DECIDER2_N},
                                               "<=", direction=WEST, copy_from_red=True))
    decider3_id = ids.next()
    entities.append(build_each_compare_decider(decider3_id, {"x": X_DECIDER3_N, "y": Y_DECIDER3_N},
                                               ">", direction=WEST, copy_from_red=False))

    # Unload wiring: any allowed cargo already on the train should unload, even if
    # LTN did not include it in the current stop output signals.
    ...
    # Request cap path: ltn-combinator emits active requests as negative item signals
    # on its green wire (ltn GREEN is directly wired to the lamp via _build_ltn_head).
    # Invert them to positive, feed the cap deciders, emit min(chest, request) onto
    # lamp RED. Lamp RED and lamp GREEN are separate networks at the lamp, so LTN
    # sums them on read but the request inverter (reading ltn GREEN) does not see
    # the cap output — no feedback loop.
    add_wire(wires, ltn_id, GREEN, req_inv_id, GREEN)
    add_wire(wires, req_inv_id, OUT_GREEN, decider2_id, GREEN)
    add_wire(wires, req_inv_id, OUT_GREEN, decider3_id, GREEN)
    add_wire(wires, decider2_id, OUT_RED, input_lamp_id, RED)
    add_wire(wires, decider3_id, OUT_RED, input_lamp_id, RED)
    ...
    # Extra pole on the combinator row — powers the request inverter at x=+2,
    # which sits outside the supply radius of the first chest-row pole at +6.5.
    extra_pole_id, extra_pole_ent = build_power_pole(ids, X_EXTRA_POLE_N, Y_EXTRA_POLE_N)
    entities.append(extra_pole_ent)
    add_wire(wires, extra_pole_id, COPPER, chest_pole_ids[0], COPPER)
    ...
    for w in range(wagons):
        ...
        for i in range(INSERTERS_PER_WAGON):
            ...
            # Output inserter with Bob's Inserters custom drop positions so pairs
            # of adjacent inserters land on opposite lanes of the belt one tile
            # north. Even i offsets +0.203125; odd i offsets -0.19921875.
            drop_x = OUTPUT_INSERTER_DROP_X_EVEN if i % 2 == 0 else OUTPUT_INSERTER_DROP_X_ODD
            iid = ids.next()
            entities.append(Entity(
                iid, "inserter", {"x": x, "y": Y_OUTPUT_INS},
                direction=SOUTH,
                drop_position=[drop_x, OUTPUT_INSERTER_DROP_Y],
            ))
            ...
            # Bob-red inserter (unload: picks wagon south, drops chest north; dir=8/SOUTH)
            bid = ids.next()
    ...
    # Chest chain RED feeds both capped-inventory deciders. No green chest chain — adding
    # a direct chest GREEN → lamp would double-count inventory.
    ...
    add_wire(wires, chest_ids[0], RED, decider3_id, RED)
...
def _build_south_compact_side(
    ...
    *,
    ...
    for w in range(wagons):
        ...
        for bob_index, offset in enumerate(SOUTH_BOB_OFFSETS):
            x = wagon_start + offset + 0.5
            cfg = SOUTH_BOB_CONFIG[offset]
            bid = ids.next()
            # Leave the middle two inserters in each south loading group unconditional.
            use_filter = bob_index in (0, 3)
            entities.append(Entity(
                entity_number=bid,
                ...
                control_behavior={"circuit_set_filters": True} if use_filter else None,
                use_filters=True if use_filter else None,
            ))
            bob_ids.append(bid)
...
def _build_provider(allowlist, wagons=3, station_name="LTN Provider"):
    ...
    ids = IDCounter()
    entities, wires = [], []
    stop_id, input_lamp_id, output_cc_id, ltn_id = _build_ltn_head(
    ...
    for w in range(wagons):
        ...
        for i in range(INSERTERS_PER_WAGON):
            x = wagon_start + i + 0.5
            iid = ids.next()
            entities.append(Entity(iid, "inserter", {"x": x, "y": Y_OUTPUT_INS}, direction=NORTH))
            cid = ids.next()
            entities.append(Entity(cid, "steel-chest", {"x": x, "y": Y_CHEST_N}))
            chest_ids.append(cid)
            bid = ids.next()
            entities.append(Entity(bid, "bob-red-inserter", {"x": x, "y": Y_UNLOAD_INS}, direction=NORTH,
                                   control_behavior={"circuit_set_filters": True}, use_filters=True))
            bob_ids.append(bid)
...
def _build_receiver(allowlist, wagons=3, station_name="LTN Receiver"):
    ...
    _build_north_side(
        ids, entities, wires, wagons,
        output_cc_id=output_cc_id, stop_id=stop_id, input_lamp_id=input_lamp_id, ltn_id=ltn_id,
...
DEFAULT_RECEIVER_PARAM_COUNT = 3


def generate_receiver(param_count=DEFAULT_RECEIVER_PARAM_COUNT, wagons=3, station_name="LTN Receiver"):
...
DEFAULT_DUAL_INPUT_COUNT = 3
DEFAULT_DUAL_OUTPUT_COUNT = 2


def generate_dual(input_count=DEFAULT_DUAL_INPUT_COUNT, output_count=DEFAULT_DUAL_OUTPUT_COUNT,
                  wagons=3, station_name="LTN Dual"):
...
def generate_output(sections: list[tuple[str, dict, str]], output_path: Optional[str] = None):
    lines = [
        ...
        "",
        "Required mods: **LTN**, **LTN Combinator Modernized** (2.0-compatible; tested",
        "against 2.2.5), **Bob's Inserters** (for `bob-red-inserter` on south compact rows).",
        "Optional: **LTN Manager** (tested against 0.5.1).",
        "",
        "---",
        "",
    ]
...
Examples:
  ...
  %(prog)s --dual --input-count 3 --output-count 2
...
--all generates a blueprint book with provider, receiver, and dual variants.
    ...
    opts.add_argument("--param-count", type=int, default=2, metavar="N",
                      help="Parameter slots for provider (default: 2) and receiver (default used only when explicitly passed)")
    opts.add_argument("--input-count", type=int, default=None, metavar="N",
                      help=f"Dual-mode input (unload) slot count (default: {DEFAULT_DUAL_INPUT_COUNT})")
    opts.add_argument("--output-count", type=int, default=None, metavar="N",
                      help=f"Dual-mode output (load) slot count (default: {DEFAULT_DUAL_OUTPUT_COUNT})")
...
def main():
    ...
    input_count = args.input_count if args.input_count is not None else DEFAULT_DUAL_INPUT_COUNT
    output_count = args.output_count if args.output_count is not None else DEFAULT_DUAL_OUTPUT_COUNT
    ...
    if args.config:
        ...
        if "receiver" in config:
            ...
            blueprints.append(generate_receiver(
                param_count=c.get("param_count", DEFAULT_RECEIVER_PARAM_COUNT),
                wagons=c.get("wagons", 3),
        ...
        if "dual" in config:
            ...
            blueprints.append(generate_dual(
                input_count=c.get("input_count", c.get("param_count", DEFAULT_DUAL_INPUT_COUNT)),
                output_count=c.get("output_count", c.get("param_count", DEFAULT_DUAL_OUTPUT_COUNT)),
                wagons=c.get("wagons", 3),
    ...
    # ─── --all mode: provider + receiver + dual (all parameterized) ───
        ...
        all_blueprints = [
            ...
            generate_receiver(param_count=DEFAULT_RECEIVER_PARAM_COUNT, wagons=3, station_name="Receiver (3W)"),
    ...
    # ─── Individual station mode ───
        ...
        if args.receiver:
            bp = generate_receiver(
                param_count=args.param_count if args.param_count != 2 else DEFAULT_RECEIVER_PARAM_COUNT,
